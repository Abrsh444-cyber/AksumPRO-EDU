...
export default function Navbar({ 
  ...
  const mobileNavItems = [
    { id: 'dashboard', label: lang === 'amh' ? 'መነሻ' : 'Home', icon: Home },
    { id: 'quiz', label: lang === 'amh' ? 'ኮርሶች' : 'Courses', icon: BookOpen },
    { id: 'aitutor', label: lang === 'amh' ? 'StudyGPT' : 'StudyGPT', icon: Sparkles, hasBadge: true },
    { id: 'universities', label: lang === 'amh' ? 'ዩኒቨርሲቲ' : 'University', icon: MapPin },
    { id: 'download', label: lang === 'amh' ? 'ፕሮፋይል' : 'Account', icon: User }
  ];

  return (
    <>
      {/* 💻 Desktop Header Navbar - Hidden on mobile, beautiful on desktop */}
      <nav id="app-main-navbar" className={`hidden lg:block sticky top-0 z-50 w-full backdrop-blur-xl transition-all duration-300 ${
        theme === 'light' 
      ...
      <div className={`lg:hidden fixed bottom-0 left-0 right-0 z-50 py-2 px-3 flex items-center justify-between shadow-2xl gap-1 transition-all duration-300 ${
        theme === 'light'
          ? 'bg-[#F9F6EE]/95 border-t border-amber-900/15 text-slate-900'
          : 'bg-slate-950/95 border-t border-vip-gold/15 text-slate-200'
      }`}>
        {mobileNavItems.map((item) => {
          ...
          const activeCircleStyle = 
            ...
            item.id === 'universities' ? 'bg-blue-500/15 text-blue-400 ring-1 ring-blue-500/35' :
            'bg-pink-500/15 text-pink-400 ring-1 ring-pink-500/35';
          ...
          return (
            <button
              ...
              key={item.id}
              onClick={() => setActiveTab(item.id)}
              className={`flex flex-col items-center justify-center gap-1 py-1 px-1 rounded-2xl transition-all cursor-pointer select-none shrink-0 flex-1 ${
                isActive 
                  ? 'scale-105 active:scale-95 text-vip-gold' 
            ...
            >
              ...
              <div className="flex items-center gap-0.5 justify-center">
                <span className={`text-[9.5px] font-black uppercase tracking-tight ${
                  isActive 
                    ? theme === 'light' ? 'text-slate-900 font-extrabold' : 'text-white font-black' 
                    : 'text-gray-400'
                }`}>
                  {item.label}
                </span>
